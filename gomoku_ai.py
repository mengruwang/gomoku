...
class gomokuAI(object):
    ...
    def has_neighbor(self, state, i, j, ):
        ...
        Neighbors are defined as pieces within 2 empty intersections.
        '''
    ...
    def has_check(self, state, i, j, ):
        ...
        for axis in directions:
            currentPattern = []
            for (xdirection, ydirection) in axis:
                currentPattern += self.direction_pattern(i, j, xdirection, ydirection, state)   # direnction_pattern returns a list of patterns
    ...
    def opponent_has_checkmate(self, state):
        ...
        '''
        Check if opponent has checkmate. five in a row
        '''

        vectors = []

        # exhaustive search
        # get all ennemy patterns in 'vectors'
        ...
        # checkmate, check if patten got has checkmate
    ...
    def evaluate_point(self, i, j):
        ...
        if j > i:
            v = [self.__gomoku.get_chessMap()[x][x + j - i] for x in range(0, N - j + i)]
            vectors.append(v)
        elif j == i:
            vectors.append([self.__gomoku.get_chessMap()[x][x] for x in range(N)])
        elif j < i:
        ...
        if i + j == N - 1:
            ...
            vectors.append([self.__gomoku.get_chessMap()[x][N - 1 - x + i + j - N + 1] for x in range(i + j - N + 1, N)])
        point_score = 0
        for v in vectors:
            score = evaluate_vector(v)
            if self.__currentState == BoardState.WHITE:
