...
class Gomoku(object):
    def __init__(self):
        ...
        self.__chessMap = [[BoardState.EMPTY for j in range(N)]
                           for i in range(N)]  # self.__chessMap  = 15*15 BoardState.EMPTY
        self.__currentI = -1
        self.__currentJ = -1
    ...
    def white_occupied(self):
        '''
        
        :return: list
        
        Get list of coordinates that white side occupieds
        '''

        whites = []
        board = self.get_chessMap()
        for i in range(len(board)):
            for j in range(len(board[i])):
                if board[i][j] == BoardState.WHITE:
                    whites.append((i, j))
        return whites

    def black_occupied(self):
        '''
